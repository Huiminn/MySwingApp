...
import com.jarby.swingApp.GUIShow.book.BookAddJFrame;
...
public class BookManagementJFrame extends JFrame implements ActionListener{
	...
	private void refreshBookManagementRendering(){
	 ...
	 public Object[][] queryData1(List<BorrowRecord> list){
		 
	        data=new Object[list.size()][head1.length];

	        for(int i=0;i<list.size();i++){
	            for(int j=0;j<head1.length;j++){
	                data[i][0]=list.get(i).getBorrowid();
	                book = bookService.findBookById(list.get(i).getBookid());
	                student = studentService.findStudentById(list.get(i).getStudentid());
	                data[i][1]=book.getBookName();
	                data[i][2]=book.getAuthor();
	                data[i][3]=list.get(i).getBorrowTime();
	                data[i][4]=list.get(i).getReturnTime();
	               
	                data[i][5]=student.getStudentName();
	                data[i][6]=student.getStudentNo();
	...
	private void borrowRecordsSearchInSmartWay(String sql,Student studentPerformed,List<BorrowRecord> borrowRecordlist){
	...
	private String assembleSqlScript(String sql,Student student){
		if(choiceStatus.getSelectedIndex() == 1)
			sql = sql + " and status='"+"borrowed"+"'";
		if(choiceStatus.getSelectedIndex() == 2)
			sql = sql + " and status='"+"returned"+"'";
		if(choiceStudentID.getSelectedIndex() == 1)
			sql = sql + " and studentid='"+student.getStudentid()+"'";
